...
fn point_in_triangle(point: &Point2D, tri: &Triangle2D) -> bool {
...
fn distance_sq((x1, y1): Point2D, (x2, y2): Point2D) -> Coord {
    let dx = x1 - x2;
    let dy = y1 - y2;
    dx * dx + dy * dy
}

fn is_ccw(tri: &Triangle2D) -> bool {
    goes_left(tri[0], tri[1], tri[2])
}

fn is_left_from_segment(
...
fn goes_left((ox, oy): Point2D, (ux, uy): Point2D, (vx, vy): Point2D) -> bool {
